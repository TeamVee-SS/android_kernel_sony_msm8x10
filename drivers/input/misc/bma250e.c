...
{
	int irq = -1;
	gpio_tlmm_config(GPIO_CFG(BMA250E_INTERRUPT, 0, GPIO_CFG_INPUT,
	...
	if (gpio_direction_input(BMA250E_INTERRUPT) < 0) {
		sDump_alert(BMA250E.debugLevel,
	...
	if (request_irq(irq, bma250e_irq, IRQF_TRIGGER_RISING, "BMA250E_SENSOR",
			data) < 0) {
...
static void bma250e_interrupt_duration(unsigned long arg)
{
	...
	case 3:
		...
		BMA250E_PMU_BW_CAMMAND =
		    0X0B; // Setting Bandwith 62.5 HZ, updated time 8ms
		break;
	case 4:
		POWER_MODE_CAMMAND = (arg > 24) ? 0X54 : 0X4C;
		BMA250E_PMU_BW_CAMMAND =
		    0X0A; // Setting Bandwith 31.25 HZ, updated time 16ms
		break;
	case 5:
		POWER_MODE_CAMMAND = (arg > 48) ? 0X56 : 0X4C;
		BMA250E_PMU_BW_CAMMAND =
		    0X09; // Setting Bandwith 15.63 HZ, updated time 32ms
		break;
	case 6:
		POWER_MODE_CAMMAND = 0X00;
		BMA250E_PMU_BW_CAMMAND =
		    0X08; // Setting Bandwith 7.81 HZ, updated time 64ms
		break;
	case 14:
...
static int bma250e_irqSetting(u8 en0, u8 en1, u8 map0, u8 map1)
{
	...
	// Reset Interrupt
	rc |=
	    i2c_smbus_write_byte_data(this_client, BMA250E_INT_RST_LATCH, 0X8B);
	return rc;
}
#endif
...
static int bma250e_type_setting(bool open)
{
	...
	if (open) {
		...
		rc |= i2c_smbus_write_byte_data(this_client, BMA250E_PMU_LPW,
						POWER_MODE_CAMMAND);
		usleep(3500);
		// Setting bandwith
		rc |= i2c_smbus_write_byte_data(this_client, BMA250E_PMU_BW,
						BMA250E_PMU_BW_CAMMAND);
#ifdef ACCELEROMETER_IRQ_USED
		switch (WorkMode) {
		case SPECIAL_MODE:
		...
		rc |= bma250e_irqSetting(irq_en[0], irq_en[1], irq_map[0],
					 irq_map[1]);
		rc |= i2c_smbus_write_byte_data(
		    this_client, BMA250E_INT_6,
		    (WorkMode != SPECIAL_MODE) ? 0X14 : 0X02);
#endif
	} else {
...
static int bma250e_enable(void)
{
	...
	(rc == 1 && WorkMode == NORMAL_MODE)
	    ? queue_delayed_work(Accelerometer_WorkQueue, &data->dw,
				 msecs_to_jiffies(10))
	    : -1;
...
static void bma250e_commandSet(Accelerometer *data, unsigned long time)
{
...
#ifdef ACCELEROMETER_IRQ_USED
	if ((time == 16384 || time < 70) &&
	    bma250e_interrupt_request(data) == true) {
	...
	if (WorkMode == NORMAL_MODE) {
		POWER_MODE_CAMMAND =
		    0X4C; // Lower power enable, sleep duration 1ms
		BMA250E_PMU_BW_CAMMAND =
		    0X0B; // Setting Bandwith 62.5 HZ, updated time 8ms
	}
...
static bool bma250e_readData(void)
{
	...
	if (i2c_smbus_read_i2c_block_data(this_client, BMA250E_ACCD_X_LSB, 6,
					  &i2cData[0]) != 6) {
	...
	ignoreCount =
	    (ignoreCount < FILTER_SIZE) ? ignoreCount + 1 : FILTER_SIZE;
#ifdef ACCELEROMETER_IRQ_USED
	if (WorkMode == SPECIAL_MODE) {
		while (queueIndex <= FILTER_INDEX) {
			memcpy(&(queueData[queueIndex]), &rawData,
...
static void bma250e_reportData(Accelerometer *data)
{
	...
	input_report_abs(data->input, ABS_Z,
			 (0 - data->sdata.Z) - data->odata.Z);
	input_sync(data->input);
	sDump_info(BMA250E.debugLevel, "%s X: %d, Y: %d, Z: %d\n", __func__,
		   data->sdata.X, data->sdata.Y, data->sdata.Z);
}
			 ...
			 const struct i2c_device_id *id)
{
	...
	if ((err = i2c_smbus_read_byte_data(client, BMA250E_BGW_CHIPID)) !=
	    0XF9) {
	...
	this_client = client;
	Accelerometer_WorkQueue =
	    create_singlethread_workqueue(input_dev->name);
	SleepTime = msecs_to_jiffies(200);
	originalTime = 200;
...
static void bma250e_shutdown(struct i2c_client *client) { bma250e_disable(); }

static struct i2c_device_id bma250e_idtable[] = {{"bma250e", 0}, {}};
