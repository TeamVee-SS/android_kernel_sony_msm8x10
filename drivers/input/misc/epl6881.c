...
enum { PLSENSOR_REG_COMMANDI = 0x00 << 3,
...
static void mailbox(const Package *const package)
{
	...
	case WLSensorConst:
		if (!err) {
			...
			memcpy(&(light->sdata.Const), package->content,
			       sizeof(int));
...
{
	return (enable) ? epl6881_enable(PROXIMITY)
...
static bool proximity_calibrate(Action action, void *buf, ssize_t size)
{
	...
	case Write:
		if (inf->job == PROXIMITY_THRESHOLD_SET) {
			...
			if (prox->enabled && !data->suspend &&
			    proximity_resetThreshold(Value + THRESHOLD_LEVEL,
						     THRESHOLD_LEVEL) >=
				THRESHOLD_LEVEL) {
				sucessful = true;
			}
	...
	case Read:
		...
		if (prox->enabled && !data->suspend) {
			memcpy(&(inf->p_infor), &(prox->sdata),
			       sizeof(ProximityInfor));
		} else {
			ProximityInfor pi = {
			    .Value = -1,
...
static bool light_calibrate(Action action, void *buf, ssize_t size)
{
	...
	case Read:
		...
		if (light->enabled && !data->suspend) {
			memcpy(&(inf->l_infor), &(light->sdata),
			       sizeof(LightInfor));
		} else {
			LightInfor li = {
			    .Value = -1,
...
static int proxmity_reportData(PLSensor *data)
{
	...
	int Threshold = (prox->sdata.State != PROXIMITY_STATE_TRUE)
			    ? prox->sdata.Threshold_H
			    : prox->sdata.Threshold_L;
	State =
	    (Value > Threshold) ? PROXIMITY_STATE_TRUE : PROXIMITY_STATE_FALSE;
	...
	if (State != prox->sdata.State) {
		if (Pneed2ChangeState == true) {
			...
			input_report_abs(prox->input, ABS_DISTANCE,
					 (State) ? PROXIMITY_DETECTED
						 : PROXIMITY_UNDETECTED);
...
static void epl6881_DST(const PLSensor *data, const int Value)
{
	...
	if (dstCount == DST_IGNORE ||
	    ((THRESHOLD_RANGE << 2) + Threshold) < Value) {
	...
	if (dstCount < DST_FILTER) {
		...
		if (dstMinValue > Threshold_L) {
			proximity_resetThreshold(dstMinValue + THRESHOLD_LEVEL,
						 THRESHOLD_LEVEL);
...
static void epl6881_autoCalibrate(const PLSensor *data, const int Value)
{
	...
	if (Value < 0 || ((Value + THRESHOLD_RANGE) > Threshold)) {
		...
		(Pneed2Reset)
		    ? proximity_resetThreshold(autoMinValue + THRESHOLD_LEVEL,
					       THRESHOLD_LEVEL)
		    : 0;
...
static bool epl6881_type_setting(WorkType wType)
{
	...
	case PROXIMITY:
		 ...
		 */
		err = i2c_smbus_write_byte_data(this_client,
						PLSENSOR_REG_COMMANDI, 0x77);
		err |= i2c_smbus_write_byte_data(this_client,
						 PLSENSOR_REG_COMMANDII, 0x51);
		err |= i2c_smbus_write_byte_data(
		    this_client, PLSENSOR_REG_GOMID_SETTING, 0XAF);
		err |= i2c_smbus_write_byte_data(
		    this_client, PLSENSOR_REG_GOLOW_SETTING, 0xAF);
		err |= i2c_smbus_write_byte_data(
		    this_client, PLSENSOR_REG_CONTROL_SETTINGI, 0x04);
		usleep(40000);
		break;
	case LIGHT:
		/**
		 ...
		 */
		err = i2c_smbus_write_byte_data(this_client,
						PLSENSOR_REG_COMMANDI, 0x92);
		err |= i2c_smbus_write_byte_data(this_client,
						 PLSENSOR_REG_COMMANDII, 0x71);
		err |= i2c_smbus_write_byte_data(
		    this_client, PLSENSOR_REG_GOMID_SETTING, 0X1E);
		err |= i2c_smbus_write_byte_data(
		    this_client, PLSENSOR_REG_GOLOW_SETTING, 0x1E);
		err |= i2c_smbus_write_byte_data(
		    this_client, PLSENSOR_REG_CONTROL_SETTINGI, 0x04);
		break;
	default:
		sDump_err(max(p_dev.debugLevel, l_dev.debugLevel),
			  "%s error case\n", __func__);
		return false;
	}
...
static bool epl6881_readData(WorkType wType)
{
	...
	default:
		sDump_err(max(p_dev.debugLevel, l_dev.debugLevel),
			  "%s error case", __func__);
		return false;
	}

	stateIsFine =
	    !(i2c_smbus_read_byte_data(this_client, PLSENSOR_REG_CHIP_STATE) &
	      cmp);
	i2cIsFine = (i2c_smbus_read_i2c_block_data(
			 this_client, PLSENSOR_REG_CH0DATA_LSB | 0x03, 4,
			 &i2cData[0]) == 4);
...
static void epl6881_errorTrans(PLSensor *data, WorkType wType)
{
	...
	case PROXIMITY: {
		...
		prox->sdata.State = PROXIMITY_STATE_UNKNOWN;
		mutex_unlock(&data->mutex);
		sDump_alert(p_dev.debugLevel,
			    "%s Proximity, try to reset chip\n", __func__);
	} break;
	case LIGHT: {
		Light *light = &data->light;
		mutex_lock(&data->mutex);
	...
	// reset chip
	i2c_smbus_write_byte_data(this_client, PLSENSOR_REG_CONTROL_SETTINGI,
				  0x00);
}
...
static void epl6881_work_func(struct work_struct *work)
{
	...
	if (atomic_read(&data->flushqueue) || data->suspend) {
		sDump_warn(max(p_dev.debugLevel, l_dev.debugLevel),
			   "%s flush queue or system had been suspend\n",
			   __func__);
		return;
	}
	...
	if (Value == 1) {
		queue_delayed_work(PLSensor_WorkQueue,
				   (light->enabled) ? &data->dw2 : &data->dw,
				   SleepTime);
	} else {
		sDump_warn(max(p_dev.debugLevel, l_dev.debugLevel),
			   "%s There has no need to re-queue\n", __func__);
	}
...
static int epl6881_probe(struct i2c_client *client,
			 const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(client->adapter,
		...
		sDump_alert(max(p_dev.debugLevel, l_dev.debugLevel),
	...
	if (err) {
		sDump_err(p_dev.debugLevel,
			  "%s input_register_device proximity error\n",
			  __func__);
		goto err_free_mem;
	}
	...
	if (err) {
		sDump_alert(l_dev.debugLevel,
			    "%s input_register_device light error\n", __func__);
		goto err_free_mem;
	}
	...
	i2c_smbus_write_byte_data(this_client, PLSENSOR_REG_CONTROL_SETTINGII,
				  0x02);
...
static struct i2c_device_id epl6881_idtable[] = {{"epl6881", 0}, {}};
