...
static ssize_t setSensorDelay(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t size)
{
	ssize_t ret = -EPERM;
	s64 delay = 0L;
...
static ssize_t getSensorDelay(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	sensor_device *sDev = dev_get_drvdata(dev);
	if (!sDev) {
			      ...
			      size_t size)
{
	...
	mutex_lock(&sDev->mutex);
	if (sDev->infor)
		siz = memory_read_from_buffer(buf, size, &pos, sDev->infor,
					      sizeof(*sDev->infor));
	else
		siz = 0;
	mutex_unlock(&sDev->mutex);
			    ...
			    size_t size)
{
	...
	mutex_lock(&sDev->mutex);
	if (sDev->calibrate && size == sizeof(Sensor_Calibrate_Infor) &&
...
static int sensor_suspend(struct device *dev, pm_message_t state) { return 0; }

static int sensor_resume(struct device *dev) { return 0; }
...
int sensor_device_register(struct device *parent, sensor_device *sDev)
{
	...
	sDev->dev =
	    device_create(sensor_class, parent, 0, sDev, "%s", sDev->name);
...
static void __exit sensor_mgr_exit(void) { class_destroy(sensor_class); }
