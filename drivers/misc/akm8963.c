...
/***** I2C I/O function ***********************************************/
static int akm8963_i2c_rxdata(struct i2c_client *i2c, unsigned char *rxData,
			      int length)
{
	struct i2c_msg msgs[] = {
	    {
		.addr = i2c->addr, .flags = 0, .len = 1, .buf = rxData,
	    },
	    {
		.addr = i2c->addr,
		.flags = I2C_M_RD,
	...
	sDump_debug(MAGNETIC.debugLevel,
		    "%s: RxData: len=%02x, addr=%02x, data=%02x\n", __func__,
		    length, addr, rxData[0]);
	return 0;
}

static int akm8963_i2c_txdata(struct i2c_client *i2c, unsigned char *txData,
			      int length)
{
	struct i2c_msg msg[] = {
	    {
		.addr = i2c->addr, .flags = 0, .len = length, .buf = txData,
	    },
	};
	...
	sDump_debug(MAGNETIC.debugLevel,
		    "%s: TxData: len=%02x, addr=%02x data=%02x\n", __func__,
...
static int AKECS_GetData(struct akm8963_data *akm, char *rbuf, int size)
{
	...
	err = wait_event_interruptible_timeout(
	    akm->drdy_wq, atomic_read(&akm->drdy), AKM8963_DRDY_TIMEOUT);
...
static void AKECS_SetYPR(struct akm8963_data *akm, int *rbuf)
{
	...
	sDump_debug(MAGNETIC.debugLevel, "AKM8963 %s: flag =0x%X\n", __func__,
		    rbuf[0]);
	sDump_debug(MAGNETIC.debugLevel,
		    "  Acceleration[LSB]: %6d,%6d,%6d stat=%d\n", rbuf[1],
		    rbuf[2], rbuf[3], rbuf[4]);
	sDump_debug(MAGNETIC.debugLevel,
		    "  Geomagnetism[LSB]: %6d,%6d,%6d stat=%d\n", rbuf[5],
...
static int AKECS_Release(struct inode *inode, struct file *file) { return 0; }
...
{
	return akm8963_sysfs_enable_show(s_akm, buf, ACC_DATA_FLAG);
}
static ssize_t akm8963_enable_acc_store(struct device *dev,
					struct device_attribute *attr,
					char const *buf, size_t count)
{
	return akm8963_sysfs_enable_store(s_akm, buf, count, ACC_DATA_FLAG);
}
...
static ssize_t akm8963_enable_ori_show(struct device *dev,
				       struct device_attribute *attr, char *buf)
{
	return akm8963_sysfs_enable_show(s_akm, buf, ORI_DATA_FLAG);
}
static ssize_t akm8963_enable_ori_store(struct device *dev,
					struct device_attribute *attr,
					char const *buf, size_t count)
{
	return akm8963_sysfs_enable_store(s_akm, buf, count, ORI_DATA_FLAG);
}
...
static ssize_t akm8963_delay_acc_show(struct device *dev,
				      struct device_attribute *attr, char *buf)
{
	return akm8963_sysfs_delay_show(s_akm, buf, ACC_DATA_FLAG);
}
static ssize_t akm8963_delay_acc_store(struct device *dev,
				       struct device_attribute *attr,
				       char const *buf, size_t count)
{
	return akm8963_sysfs_delay_store(s_akm, buf, count, ACC_DATA_FLAG);
}
...
static ssize_t akm8963_delay_ori_show(struct device *dev,
				      struct device_attribute *attr, char *buf)
{
	return akm8963_sysfs_delay_show(s_akm, buf, ORI_DATA_FLAG);
}
static ssize_t akm8963_delay_ori_store(struct device *dev,
				       struct device_attribute *attr,
				       char const *buf, size_t count)
{
	return akm8963_sysfs_delay_store(s_akm, buf, count, ORI_DATA_FLAG);
}

/***** accel (binary) ***/
static ssize_t akm8963_bin_accel_write(struct file *file, struct kobject *kobj,
...
{
	int irq = 0;
	gpio_tlmm_config(GPIO_CFG(AKM8963_IRQ, 0, GPIO_CFG_INPUT,
	...
	if (gpio_direction_input(AKM8963_IRQ) < 0) {
		sDump_alert(MAGNETIC.debugLevel,
			    "%s failed to config the interrupt pin\n",
			    __func__);
	...
	if (request_threaded_irq(irq, NULL, akm8963_irq,
...
int akm8963_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (pdata) {
		...
		/* Platform data is not available.
		   Layout and Outbit information should be set by each
		   application. */
	...
	err |= create_device_attributes(MAGNETIC.dev, akm8963_attributes);
	err |= create_device_binary_attributes(&(MAGNETIC.dev->kobj),
					       akm8963_bin_attributes);
	if (0 > err) {
		sDump_err(MAGNETIC.debugLevel, "%s: create sysfs failed.\n",
			  __func__);
		goto exit6;
	}
...
static int akm8963_remove(struct i2c_client *client)
{
	...
	remove_device_binary_attributes(&(MAGNETIC.dev->kobj),
					akm8963_bin_attributes);
	remove_device_attributes(MAGNETIC.dev, akm8963_attributes);
	sensor_device_unregister(&MAGNETIC);
...
static const struct i2c_device_id akm8963_id[] = {{AKM8963_I2C_NAME, 0}, {}};

static const struct dev_pm_ops akm8963_pm_ops = {
    .suspend = akm8963_suspend, .resume = akm8963_resume,
};
